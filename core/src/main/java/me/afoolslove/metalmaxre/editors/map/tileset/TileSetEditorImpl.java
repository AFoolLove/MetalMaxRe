...
public class TileSetEditorImpl extends RomBufferWrapperAbstractEditor implements ITileSetEditor {
     ...
     * 世界地图图块的组合数据，全局固定
     */
    private byte[][][] worldCombinations = new byte[0x03][0x40][0x04];
    /**
     * 世界地图图块的特性和调色板
     */
    private byte[][] worldAttributes = new byte[0x03][0x40];
    ...
    public TileSetEditorImpl(@NotNull MetalMaxRe metalMaxRe) {
        this(metalMaxRe,
                ...
                DataAddress.fromPRG(0x0FEAA - 0x10));
    }
...
//        if (colors == null) {
//            IPaletteEditor paletteEditor = getMetalMaxRe().getEditorManager().getEditor(IPaletteEditor.class);
//            List<PaletteRow> palettes = paletteEditor.getPalettes().get(mapProperties.palette);
